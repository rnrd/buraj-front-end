...
import {getSepetKurpChartDataFromApi} from "../redux/actions/Actions";
...
function SepetKurpChart(props) {
    ...
    const sepetKurpChartDispatch=useDispatch();

    const getResult=(data)=>{

        //console.log(data);
        sepetKurpChartDispatch(getSepetKurpChartDataFromApi(data))
    }

    const result=useSelector(state=>state.sepetKurpChartReducer,shallowEqual);
    console.log(result);

    /*var kurpBoyu=0.0;
    for(let i=0;i<postData.L.length; i++){
        kurpBoyu=kurpBoyu+postData.L[i];
    }*/

    var kurpBoyu=postData.L.reduce((accumulator,value)=>{return accumulator+value;},0);
